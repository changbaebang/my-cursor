...
description: >-
  Slack 메시지·스레드 링크와 GitHub PR을 연결해 **diff 기반** 코드 리뷰를 수행하고, 완료 시 Slack **원본
  메시지에** :eyes: / :white_check_mark: **리액션**을 남기고 스레드에 리뷰 요약을
  답글합니다. Approve 전 diff 신호 스캔·논쟁 소지 변경은 GitHub에 1문장 설명(부드러운 톤 ≠ 얕은 리뷰).
  **이미 MERGED/CLOSED된 PR은 철수**(Slack·GitHub 미게시). "슬랙 PR 리뷰", "slack review", "슬랙 메시지 리뷰", "PR 리뷰 부탁",
  "리뷰 부드럽게", slack permalink, 리뷰 완료 이모지, check 리액션 요청 시 사용합니다.
---
...
| 항목 | 내용 |
|------|------|
| **로그인** | `gh auth login` (keyring, 계정 `changbaebang-29cm` 등) |
...
# 또는
~/.cursor/scripts/cb/gh-with-keyring.sh pr view 6966 --repo 29CM-Developers/frontend-29cm-platform
```
...
```bash
export GH_TOKEN="$(env -u GH_TOKEN -u GITHUB_AUTH_TOKEN gh auth token)"
gh api repos/<owner>/<repo>/pulls/<N>/requested_reviewers -f 'reviewers[]=changbaebang-29cm'
...
**리뷰 맵:** [`../pr-review/REVIEW-MAP.md`](../pr-review/REVIEW-MAP.md)
...
3. 스레드에서 PR 번호·repo 추출: `pull/6953`, `github.com/.../pull/N` 등
...
- `author.login` → `hongdongkim-29cm` 등. 표시 이름은 Slack/GitHub 프로필과 다를 수 있음.
...
### 4.1 Diff 코드 리뷰 — Approve 전 필수 (§diff-review-checklist)

**「부드럽게」「가볍게」 요청 = Slack 톤만 완화. diff 점검·GitHub 설명 의무는 동일.**

1. [`diff-review-checklist.md`](./diff-review-checklist.md)의 **Approve 전 최소 바** + **신호 스캔** (`scan-diff-signals.mjs` 또는 grep) 실행
2. mock 제거·스키마·공유 컴포넌트·null/동등 비교 변경은 **해당 hunk**를 읽는다
3. 팀원이 질문할 법한 한 줄 변경(`!= null`, `couponSource`→`isBrandCartCoupon` 등)은 **Approve여도 GitHub에 의도 1문장** — 코멘트 없이 통과 금지
4. PR 본문 테스트 체크리스트가 비어 있으면 🟡 (non-blocking) — Request changes 아님

| 실패 패턴 | 예 (#7088) |
|-----------|------------|
| 삭제 300줄 + 추가 60줄 → signal scan 생략 | `!== null`→`!= null` 변경에 설명 없이 Approve → 사후 「이걸 승인해?」 |
| PR Oracle·스크린샷만 보고 diff 미확인 | mock 제거는 맞았으나 리뷰 신뢰만 손상 |

**PR 메타**(제목·Tier·라벨)만 볼 때는 `29CM-fe` `pr-review` 스킬. **코드 본문**은 여기 + checklist.

### 4.2 리뷰 작성 (한국어)

- GitHub·Slack 게시 전 **PR별 `author` 확인** (§1.2). 멘션된 사람 이름으로 감사 인사 **금지** (확인 없이).
- 복수 PR이면 PR마다 작성자·총평을 분리한다.
- **GitHub**와 **Slack 답글** 분량·톤은 다르다 (§5.2, §5.3). GitHub = diff·의도·🟡, Slack = 짧게.

**GitHub** (`gh pr review` / comment) — checklist 템플릿 우선 ([§ Approve 본문](./diff-review-checklist.md#github-approve-본문-템플릿-최소)):
...
Slack은 스레드에 이미 PR 링크가 있는 경우가 많다. **Approve = 보통 잘 됐다는 뜻**이므로, 뻔한 「잘 된 점」 bullet은 **쓰지 않는다.** diff·의도·🟡 상세는 **GitHub**에 남긴다.
...
### 5.3 Slack 답글 — 「부드럽게」 요청 시

| 채널 | 부드러운 톤에서도 |
|------|-------------------|
| GitHub | §4.1 signal에 걸린 변경은 **반드시** 1문장 설명 (칭찬 bullet으로 대체 금지) |
| Slack | §5.2 유지 — 공격적 표현·장황한 지적 금지, 🟡은 「한 번만 확인 부탁」 정도 |
...
- **부드럽게 ≠ rubber-stamp** — §4.1 diff checklist 생략 금지. 논쟁 소지 변경은 GitHub에만 짧게 남긴다 (§5.3).
- **사후 제보** (`이걸 승인해?`) — [`diff-review-checklist.md` § 사후 제보](./diff-review-checklist.md#사후-제보-대응) follow-up comment, checklist 보강.
...
| | |
|--|--|
| **팀 규칙** | runner는 **29CM self-hosted**만 사용 (「29 self-hosted」) |
| **금지** | GitHub-hosted (`ubuntu-latest` 등) |
| **라벨 예** | `small-29cm`, `default-29cm`, `default-4xlarge-29cm` — 레포 기존 워크플로와 동일 계열 |
