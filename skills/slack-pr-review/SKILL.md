...
# 또는
~/.cursor/scripts/cb/gh-with-keyring.sh pr view 6966 --repo your-org/your-frontend-monorepo
```
...
3. 스레드에서 PR 번호·repo 추출: `pull/1234`, `github.com/.../pull/N` 등
...
- `author.login` → `contributor-example` 등. 표시 이름은 Slack/GitHub 프로필과 다를 수 있음.
...
```bash
node "$HOME/.cursor/skills/slack-pr-review/scripts/gather-pr-context.mjs" --pr 1234
```
...
node "$HOME/.cursor/skills/slack-pr-review/scripts/run.mjs" \
  --url "<permalink>" --pr 1234 --react-both
```
